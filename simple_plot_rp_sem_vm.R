...
for(region_index in 1:length(region_list)){
  ...
  for(unit_num in unit_list){
    ...
    ##############
    #value
    png(paste("hand_paper_nl_sem_2.5txt_",region_list[region_index],"_v_unit_",unit_num,".png",sep=""),width=8,height=6,units="in",res=500)

    ######gonna have to manualy set these, make the limits slightly greater than the vals where necessary. That way can be even, all 3 values. Or decrease the ticks and increase the limits
    ...
    plt_cue <- plt_cue + theme(legend.position="none",axis.title=element_blank(),axis.text.y=element_text(size=rel(1.5)),axis.text.x=element_blank(),plot.title=element_text(size=rel(2)))
    plt_cue <- plt_cue + theme(axis.text.y=element_text(size=rel(2.5))) + scale_y_continuous(breaks=c(-0.4,0.0,0.4),limits=c(-0.7,0.7))
    ...
    plt_res <- plt_res + theme(legend.position="none",axis.title=element_blank(),axis.text.y=element_text(size=rel(1.5)),axis.text.x=element_blank(),plot.title=element_text(size=rel(2)))
    plt_res <- plt_res + theme(axis.text.y=element_text(size=rel(2.5))) + scale_y_continuous(breaks=c(0.0,0.4,0.8),limits=c(-0.4,1.0))
  
    ggarrange(plt_cue,plt_res,ncol=1)
    graphics.off()

    # motivation (final)
    png(paste("hand_paper_nl_sem_2.5txt_",region_list[region_index],"_m_unit_",unit_num,".png",sep=""),width=8,height=6,units="in",res=500)
    ...
    plt_cue <- plt_cue + theme(legend.position="none",axis.title=element_blank(),axis.text.y=element_text(size=rel(1.5)),axis.text.x=element_blank(),plot.title=element_text(size=rel(2)))
    plt_cue <- plt_cue + theme(axis.text.y=element_text(size=rel(2.5))) + scale_y_continuous(breaks=c(0.0,0.3,0.6),limits=c(-0.4,0.8))
    ...
    plt_res <- plt_res + theme(legend.position="none",axis.title=element_blank(),axis.text.y=element_text(size=rel(1.5)),axis.text.x=element_blank(),plot.title=element_text(size=rel(2)))
    plt_res <- plt_res + theme(axis.text.y=element_text(size=rel(2.5))) + scale_y_continuous(breaks=c(0.0,0.4,0.8),limits=c(-0.4,1.2))
