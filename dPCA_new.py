        ...
        def compute_mean_score(X,trialX,n_splits):
            ...
            for shuffle in range(n_splits):
                ...
                for key in keys:
                    ...
                    for ax_var in (axes):
                       # print(ax_var)
                        if not np.shape(np.shape(trainZ[key])) >= (ax_var+1):
                            trainZ[key] = np.mean(trainZ[key],axis=ax_var+1)
                            validZ[key] = np.mean(validZ[key],axis=ax_var+1)
                            #trainZ[key] = np.mean(trainZ[key],axis=ax_var)
                            #validZ[key] = np.mean(validZ[key],axis=ax_var)
