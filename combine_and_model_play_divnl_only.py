...
def make_div_nl_avg_response_together_model(fr_data_dict,region_key,type_key,file_length,avg_and_corr,total_unit_num):
        ...
        combined_dict = {'params':params,'covs':covs,'perr':perr,'ss_res':ss_res,'AIC':AIC}        
        return_dict = {'fit_params':fit_params[0:sig_unit_length,:],'cov_total':cov_total[0:sig_unit_length,:],'perr_total':perr_total[0:sig_unit_length,:],'AIC_combined':AIC,'ss_res_total':np.trim_zeros(ss_res_total,'b'),'AIC_total':AIC_total[0:sig_unit_length],'combined':combined_dict,'AIC_overall':0,'num_units_no_fit':num_units_no_fit,'perc_units_no_fit':num_units_no_fit/float(sig_unit_length),'total_num_units':sig_unit_length,'p_val_total':p_val_total[0:sig_unit_length],'F_val_total':F_val_total[0:sig_unit_length],'r_sq_total':r_sq_total[0:sig_unit_length],'AIC_sig_model':AIC_sig_model,'AIC_sig_avg':AIC_sig_avg,'num_sig_fit':num_sig_fit,'r_sq_avg':r_sq_avg,'AIC_new_total':AIC_new_total,'r_sq_adj_total':r_sq_adj_total,'AIC_lr_total':AIC_lr_total,'AICc_total':AICc_total,'mse_total':mse_total,'r_sq_adj_avg':r_sq_adj_avg}
...
for region_key,region_val in data_dict_all.iteritems():
    ...
    #TODO
    print 'div nl noe JKJK avg response together'   
    ...
    for AIC_type in AIC_type_names:
        ...
        for win_key,win_val_x in div_nl_separate_multiply_model_return.iteritems():
            delAIC = make_delta_AIC(np.column_stack((div_nl_model_return[win_key][AIC_type],div_nl_noe_model_return[win_key][AIC_type],div_nl_Y_model_return[win_key][AIC_type],div_nl_separate_add_model_return[win_key][AIC_type],div_nl_separate_multiply_model_return[win_key][AIC_type])),np.column_stack((div_nl_model_return[win_key]['p_val_total'],div_nl_noe_model_return[win_key]['p_val_total'],div_nl_Y_model_return[win_key]['p_val_total'],div_nl_separate_add_model_return[win_key]['p_val_total'],div_nl_separate_multiply_model_return[win_key]['p_val_total'])))
            ...
            if ceil_val != 0:
                ...
                try:
                    axarr[0].hist(delAIC['dAICs_sig_any'][:,0][~np.isnan(delAIC['dAICs_sig_any'][:,0])],color='slateblue',lw=0,bins=12)
                except:
                    pass
                axarr[0].set_title('div nl',fontsize='small')
                try:
                ...
                try:
                    ...
                    pass
                axarr[3].set_title('div nl avg response separate',fontsize='small')
                try:
            ...
            if ceil_val != 0:
                ...
                try:
                    ...
                    axarr[1].set_title('div nl avg response rogether',fontsize='small')
                except:
                    y1 = 0
                    pass
                try:
                ...
                max_y = int(np.max(np.append(np.append(np.append(np.append(y0,y1),y2),y3),y4)))
                for axi in axarr.reshape(-1):
                    try:
                        if max_y > 3:
                            axi.yaxis.set_major_locator(plt.MaxNLocator(3))
            ...
            try:
                axarr[0].hist(delAIC['akaike_weights'][:,0][~np.isnan(delAIC['akaike_weights'][:,0])],color='crimson',lw=0,bins=12)
