...
for (region_index in 1:length(region_list)){
  ...
  if (region_index==1){
    ...
    slopes_bfr_cue_sigall <- PmD_slopes_bfr_cue_sigall
  ...
  ##################
  #Both sig. Do another w/ sig alpha then sig beta. Another with sig either or?
  
  aft_cue_sig <- slopes_aft_cue[slopes_aft_cue[slopes_aft_cue$beta_p <= 0.05,]$alpha_p <= 0.05,]
  bfr_res_sig <- slopes_bfr_result[slopes_bfr_result[slopes_bfr_result$beta_p <= 0.05,]$alpha_p <= 0.05,]
  aft_res_sig <- slopes_aft_result[slopes_aft_result[slopes_aft_result$beta_p <= 0.05,]$alpha_p <= 0.05,]
  
  png(paste('coeffs_slope_collated_hist_',region_list[region_index],'.png',sep=""),width=8,height=6,units="in",res=500)
  
  if (length(aft_cue_sig$alpha) == 0){ac_alpha <- data.frame(slope=0)
  }else{ac_alpha <- data.frame(slope=aft_cue_sig$alpha)}
  if (length(aft_cue_sig$beta) == 0){ac_beta <- data.frame(slope=0)
  }else{ac_beta <- data.frame(slope=aft_cue_sig$beta)}
  ac_alpha$name <- 'alpha'
  ac_beta$name <- 'beta'
  
  if (length(bfr_res_sig$alpha) == 0){br_alpha <- data.frame(slope=0)
  }else{br_alpha <- data.frame(slope=bfr_res_sig$alpha)}
  if (length(bfr_res_sig$beta) == 0){br_beta <- data.frame(slope=0)
  }else{br_beta <- data.frame(slope=bfr_res_sig$beta)}
  br_alpha$name <- 'alpha'
  br_beta$name <- 'beta'
  
  if (length(aft_res_sig$alpha) == 0){ac_alpha <- data.frame(slope=0)
  }else{ar_alpha <- data.frame(slope=aft_res_sig$alpha)}
  if (length(aft_res_sig$beta) == 0){ar_beta <- data.frame(slope=0)
  }else{ar_beta <- data.frame(slope=aft_res_sig$beta)}  
  ar_alpha$name <- 'alpha'
  ar_beta$name <- 'beta'
  ...
  multiplot(ac_plt,br_plt,ar_plt,cols=1)
  graphics.off()
  ...
  png(paste('coeffs_either_slope_collated_hist_',region_list[region_index],'.png',sep=""),width=8,height=6,units="in",res=500)
  
  ac_alpha <- data.frame(slope= slopes_aft_cue[slopes_aft_cue$alpha_p <= 0.05,]$alpha)
  ac_beta <- data.frame(slope= slopes_aft_cue[slopes_aft_cue$beta_p <= 0.05,]$beta)
  #if (length(aft_cue_sig$alpha)==0){ac_alpha <- data.frame(slope=NA)}
  #if (length(aft_cue_sig$beta)==0){ac_beta <- data.frame(slope=NA)}
  ac_alpha$name <- 'alpha'
  ac_beta$name <- 'beta'
  
  br_alpha <- data.frame(slope= slopes_bfr_result[slopes_bfr_result$alpha_p <= 0.05,]$alpha)
  br_beta <- data.frame(slope= slopes_bfr_result[slopes_bfr_result$beta_p <= 0.05,]$beta)
  #if (length(bfr_res_sig$alpha)==0){br_alpha <- data.frame(slope=NaN)}
  #if (length(bfr_res_sig$beta)==0){br_beta <- data.frame(slope=NaN)}
  br_alpha$name <- 'alpha'
  br_beta$name <- 'beta'
  
  ar_alpha <- data.frame(slope= slopes_aft_result[slopes_aft_result$alpha_p <= 0.05,]$alpha)
  ar_beta <- data.frame(slope= slopes_aft_result[slopes_aft_result$beta_p <= 0.05,]$beta)
  #if (length(aft_res_sig$alpha)==0){ar_alpha <- data.frame(slope=NaN)}
  #if (length(aft_res_sig$beta)==0){ar_beta <- data.frame(slope=NaN)}
  ar_alpha$name <- 'alpha'
  ar_beta$name <- 'beta'
