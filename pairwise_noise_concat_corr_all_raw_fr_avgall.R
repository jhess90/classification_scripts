...
compute_plot_corr <- function(all_cue_fr_M1,all_cue_fr_S1,all_cue_fr_PmD,all_res_fr_M1,all_res_fr_S1,all_res_fr_PmD,total_unit_num_M1,total_unit_num_S1,total_unit_num_PmD,type_name,trial_inds){
  ...
  #
  avg_psth_cue_M1 <- apply(all_cue_fr_M1,c(1,3),mean)
  avg_psth_res_M1 <- apply(all_res_fr_M1,c(1,3),mean)
  ...
  for(unit_num in 1:total_unit_num_M1){
    if (length(trial_inds) > 1){demeaned_cue_avgs_M1 <- rbind(demeaned_cue_avgs_M1,as.vector(t(demeaned_cue_fr_M1[unit_num,,])))}else if (length(trial_inds) == 1){demeaned_cue_avgs_M1 <- rbind(demeaned_cue_avgs_M1,demeaned_cue_fr_M1[unit_num,])}
    if (length(trial_inds) > 1){demeaned_res_avgs_M1 <- rbind(demeaned_res_avgs_M1,as.vector(t(demeaned_res_fr_M1[unit_num,,])))}else if (length(trial_inds) == 1){demeaned_res_avgs_M1 <- rbind(demeaned_res_avgs_M1,demeaned_res_fr_M1[unit_num,])}
  }
  
  #
  avg_psth_cue_S1 <- apply(all_cue_fr_S1,c(1,3),mean)
  avg_psth_res_S1 <- apply(all_res_fr_S1,c(1,3),mean)
  ...
  for(unit_num in 1:total_unit_num_S1){
    if (length(trial_inds) > 1){demeaned_cue_avgs_S1 <- rbind(demeaned_cue_avgs_S1,as.vector(t(demeaned_cue_fr_S1[unit_num,,])))}else if (length(trial_inds) == 1){demeaned_cue_avgs_S1 <- rbind(demeaned_cue_avgs_S1,demeaned_cue_fr_S1[unit_num,])}
    if (length(trial_inds) > 1){demeaned_res_avgs_S1 <- rbind(demeaned_res_avgs_S1,as.vector(t(demeaned_res_fr_S1[unit_num,,])))}else if (length(trial_inds) == 1){demeaned_res_avgs_S1 <- rbind(demeaned_res_avgs_S1,demeaned_res_fr_S1[unit_num,])}
  }
  
  #
  avg_psth_cue_PmD <- apply(all_cue_fr_PmD,c(1,3),mean)
  avg_psth_res_PmD <- apply(all_res_fr_PmD,c(1,3),mean)
  ...
  for(unit_num in 1:total_unit_num_PmD){
    if (length(trial_inds) > 1){demeaned_cue_avgs_PmD <- rbind(demeaned_cue_avgs_PmD,as.vector(t(demeaned_cue_fr_PmD[unit_num,,])))}else if (length(trial_inds) == 1){demeaned_cue_avgs_PmD <- rbind(demeaned_cue_avgs_PmD,demeaned_cue_fr_PmD[unit_num,])}
    if (length(trial_inds) > 1){demeaned_res_avgs_PmD <- rbind(demeaned_res_avgs_PmD,as.vector(t(demeaned_res_fr_PmD[unit_num,,])))}else if (length(trial_inds) == 1){demeaned_res_avgs_PmD <- rbind(demeaned_res_avgs_PmD,demeaned_res_fr_PmD[unit_num,])}
  }
  ...
  write.table(cue_sum_label_df,paste('raw_noise_concat_',type_name,'_avgall_cue.txt',sep=""),sep="\t",row.names=FALSE)
  write.table(res_sum_label_df,paste('raw_noise_concat_',type_name,'_avgall_res.txt',sep=""),sep="\t",row.names=FALSE)
  
  #plot correlation matrix
  png(paste("raw_noise_concat_corr_spear_all_avgall_",type_name,".png",sep=""),width=8,height=6,units="in",res=500)
  ...
  #plot correlation matrix
  png(paste("raw_noise_concat_corr_spear_all_no_txt_avgall_",type_name,".png",sep=""),width=8,height=6,units="in",res=500)
