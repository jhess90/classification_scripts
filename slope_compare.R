...
for (region_index in 1:length(file_list)){
  ...
  #df_bfr_cue.m <- melt(df_bfr_cue)
  #df_aft_cue.m <- melt(df_aft_cue)
  #df_bfr_result.m <- melt(df_bfr_result)
  #df_aft_result.m <- melt(df_aft_result)
...
plot_newmv <- function(mv_array,region_key,type_key){
  ...
  for (unit_ind in 1:dim(mv_array)[1]){
    ...
    png(paste(region_list[region_ind],'_',type_key,'_unit',unit_num,'_mtv_val_plt.png',sep=""),width=8,height=6,units="in",res=500)
  ...
  return_list <- list(val = val_list,mtv = mtv_list,perc_v = perc_val_sig,perc_m = perc_mtv_sig,sig_slopes=sig_slopes,all_slopes=all_slopes)
  return(return_list)
}
...
for (region_ind in 1:length(region_list)){
  ...
  bfr_cue_sig_zero_slope <- bfr_cue_list$sig_slopes[(bfr_cue_list$sig_slopes$slopes >= -0.1 && bfr_cue_list$sig_slopes$slopes <= 0.1),]
  ...
  aft_cue_sig_zero_slope <- aft_cue_list$sig_slopes[(aft_cue_list$sig_slopes$slopes >= -0.1 && aft_cue_list$sig_slopes$slopes <= 0.1),]
  ...
  bfr_result_sig_zero_slope <- bfr_result_list$sig_slopes[(bfr_result_list$sig_slopes$slopes >= -0.1 && bfr_result_list$sig_slopes$slopes <= 0.1),]
  ...
  aft_result_sig_zero_slope <- aft_result_list$sig_slopes[(aft_result_list$sig_slopes$slopes >= -0.1 && aft_result_list$sig_slopes$slopes <= 0.1),]
  ...
  bfr_cue_all_zero_slope <- bfr_cue_list$all_slopes[(bfr_cue_list$all_slopes$slopes >= -0.1 && bfr_cue_list$all_slopes$slopes <= 0.1),]
  ...
  aft_cue_all_zero_slope <- aft_cue_list$all_slopes[(aft_cue_list$all_slopes$slopes >= -0.1 && aft_cue_list$all_slopes$slopes <= 0.1),]
  ...
  bfr_result_all_zero_slope <- bfr_result_list$all_slopes[(bfr_result_list$all_slopes$slopes >= -0.1 && bfr_result_list$all_slopes$slopes <= 0.1),]
  ...
  aft_result_all_zero_slope <- aft_result_list$all_slopes[(aft_result_list$all_slopes$slopes >= -0.1 && aft_result_list$all_slopes$slopes <= 0.1),]
  ...
  png(paste('linreg_',region_list[region_ind],'.png',sep=""),width=8,height=6,units="in",res=500)
