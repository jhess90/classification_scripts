...
for key,value in data_dict.iteritems():
	...
	aft_res_bins = int(aft_result_time * 1000/bin_size)
        ...
        temp = binned_data['smoothed']

	all_cue_fr = np.zeros((np.shape(temp)[0],np.shape(condensed)[0],-1*bfr_cue_bins + aft_cue_bins))
	all_res_fr = np.zeros((np.shape(temp)[0],np.shape(condensed)[0],-1*bfr_res_bins + aft_res_bins))
	
        for unit_num in range(np.shape(temp)[0]):
                for i in range(np.shape(condensed)[0]):
                        all_cue_fr[unit_num,i,:] = temp[unit_num,condensed[i,8] + bfr_cue_bins : condensed[i,8] + aft_cue_bins]
                        all_res_fr[unit_num,i,:] = temp[unit_num,condensed[i,9] + bfr_res_bins : condensed[i,9] + aft_res_bins]
        
        if(subset_corr_bool):
                subset = np.zeros((np.shape(temp)[0],np.shape(condensed)[0],aft_cue_bins + -1*bfr_res_bins + aft_res_bins))

                subset[:,:,0:aft_cue_bins] = all_cue_fr[:,:,-1*bfr_res_bins:]
                subset[:,:,aft_cue_bins : ] = all_res_fr

                subset_reshaped = np.reshape(subset,(np.shape(subset)[0],np.shape(subset)[1] * np.shape(subset)[2]))
        ...
        if(subset_corr_bool):
                corr_data = run_corr(subset_reshaped,condensed)
        
        elif(gaussian_bool or zscore_bool):
