...
def div_nl_avg_response_separate_func(x,a,b,c,d,e):
        ...
        return (r + a) / (sum_r + r + b) + (p + c) / (sum_p + p + d) + e
...
def ronly_func(x,a,b,c):
        ...
        r,p,sum_r = x
        return a*r / (r + sum_r + b) + c


def ponly_func(x,a,b,c):
        r,p,sum_p = x

        return a*p / (p + sum_p + b) + c
...
#####
#divide by sum of activity for different r/p types. One together, one seperate and added
#remove separate multipy
#remove noe
...
def plot_fr_and_model(unit_num,fr_r_p,params,model_type,region_key,type_key):
        ...
        if model_type == 'linear':
        ...
        elif model_type == 'div_nl_avg_response_together':
            ...
            z = ronly_func([x,y,params[3]],params[0],params[1],params[2])
        elif model_type == 'ponly':
            z = ponly_func([x,y,params[3]],params[0],params[1],params[2])
...
####################
def make_div_nl_avg_response_together_model(fr_data_dict,region_key,type_key,file_length,avg_and_corr,total_unit_num):
        #set each model
        num_params = 4
        ...
        n_total = np.zeros((file_length))
        #
        all_r_p_responses = np.zeros((total_unit_num,2))
        ...
        for i in range(file_length):
            ...
            for unit_num in range(avg_fr_data.shape[1]):
                ...
                #
                #all_r_p_responses = [avg_fr_by_r,avg_fr_by_p]
                model_out = div_nl_avg_response_together_func([r_vals,p_vals,avg_fr_by_r,avg_fr_by_p],params[0],params[1],params[2],params[3])
        ...
        if num_sig_fit > 0:
            ...
            for i in range(num_sig_fit):
                ...
                if unit < unit_ct:
                    if plot_bool:
                        try:
                            plot_fr_and_model(unit,fr_r_p_list[unit],np.append(fit_params[unit],all_r_p_responses[unit,:]),'div_nl_avg_response_together',region_key,type_key)
                        except:
                            pdb.set_trace()
            ...
            if plot_hist_bool:
                ...
                plt.tight_layout()
                plt.subplots_adjust(top=0.9,bottom=0.05,left=0.05)
                plt.suptitle('Avg Response Together: %s %s' %(region_key,type_key))
                plt.rcParams['xtick.labelsize'] = 8
                plt.rcParams['ytick.labelsize'] = 8

                plt.savefig('AIC_%s_%s_avg_response_together'  %(region_key,type_key))
                plt.clf()
                ...
                plt.savefig('R2_MSE_%s_%s_avg_response_together'  %(region_key,type_key))
                plt.clf()
...
def make_div_nl_avg_response_separate_model(fr_data_dict,region_key,type_key,file_length,avg_and_corr,total_unit_num):
        #set each model
        num_params = 5
        ...
        for i in range(file_length):
            ...
            for unit_num in range(avg_fr_data.shape[1]):
                ...
                #
                #all_r_p_responses = [avg_fr_by_r,avg_fr_by_p]
                model_out = div_nl_avg_response_separate_func([r_vals,p_vals,avg_fr_by_r,avg_fr_by_p],params[0],params[1],params[2],params[3],params[4])
        ...
        if num_sig_fit > 0:
            ...
            for i in range(num_sig_fit):
                ...
                if unit < unit_ct:
                    if plot_bool:
                        try:
                            plot_fr_and_model(unit,fr_r_p_list[unit],np.append(fit_params[unit],all_r_p_responses[unit,:]),'div_nl_avg_response_separate',region_key,type_key)
                        except:
                            pdb.set_trace()
            ...
            if plot_hist_bool:
                ...
                plt.tight_layout()
                plt.subplots_adjust(top=0.9,bottom=0.05,left=0.05)
                plt.suptitle('Avg Response Separate: %s %s' %(region_key,type_key))
                plt.rcParams['xtick.labelsize'] = 8
                plt.rcParams['ytick.labelsize'] = 8

                plt.savefig('AIC_%s_%s_avg_response_separate'  %(region_key,type_key))
                plt.clf()
                ...
                plt.savefig('R2_MSE_%s_%s_avg_response_separate'  %(region_key,type_key))
                plt.clf()
...
def make_ronly_model(fr_data_dict,region_key,type_key,file_length,avg_and_corr,total_unit_num):
        ...
        for i in range(file_length):
            ...
            for unit_num in range(avg_fr_data.shape[1]):
                ...
                #firing rate = a*R + b*P + c
                try:
                    params,covs = curve_fit(ronly_func,[r_vals,p_vals,avg_fr_by_r],avg_frs)
                except:
                    #fake very high values
                    AIC_total[unit_ct] = np.nan
                    ss_res_total[unit_ct] = np.nan
                ...
                model_out = ronly_func([r_vals,p_vals,avg_fr_by_r],params[0],params[1],params[2])
                residuals = avg_frs - model_out
                ss_res = np.sum(residuals**2)
                k = num_params
                AIC = 2*k - 2*np.log(ss_res)  #np.log(x) = ln(x)
...
def make_ponly_model(fr_data_dict,region_key,type_key,file_length,avg_and_corr,total_unit_num):
        ...
        for i in range(file_length):
            ...
            for unit_num in range(avg_fr_data.shape[1]):
                ...
                #firing rate = a*R + b*P + c
                try:
                    params,covs = curve_fit(ponly_func,[r_vals,p_vals,avg_fr_by_p],avg_frs)
                except:
                    #fake very high values
                    AIC_total[unit_ct] = np.nan
                    ss_res_total[unit_ct] = np.nan
                ...
                model_out = ponly_func([r_vals,p_vals,avg_fr_by_p],params[0],params[1],params[2])
                residuals = avg_frs - model_out
                ss_res = np.sum(residuals**2)
                k = num_params
                AIC = 2*k - 2*np.log(ss_res)  #np.log(x) = ln(x)
...
for region_key,region_val in data_dict_all.iteritems():
    ...
    print 'linear'   
    div_nl_aft_cue_model = make_lin_model(file_dict,region_key,'aft_cue',file_length,avg_and_corr,total_unit_num)
    div_nl_bfr_res_model = make_lin_model(file_dict,region_key,'bfr_res',file_length,avg_and_corr,total_unit_num)
    div_nl_aft_res_model = make_lin_model(file_dict,region_key,'aft_res',file_length,avg_and_corr,total_unit_num)
    div_nl_res_win_model = make_lin_model(file_dict,region_key,'res_win',file_length,avg_and_corr,total_unit_num)
    div_nl_concat_model = make_lin_model(file_dict,region_key,'concat',file_length,avg_and_corr,total_unit_num)
    ...
    print 'diff'   
    div_nl_noe_aft_cue_model = make_diff_model(file_dict,region_key,'aft_cue',file_length,avg_and_corr,total_unit_num)
    div_nl_noe_bfr_res_model = make_diff_model(file_dict,region_key,'bfr_res',file_length,avg_and_corr,total_unit_num)
    div_nl_noe_aft_res_model = make_diff_model(file_dict,region_key,'aft_res',file_length,avg_and_corr,total_unit_num)
    div_nl_noe_res_win_model = make_diff_model(file_dict,region_key,'res_win',file_length,avg_and_corr,total_unit_num)
    div_nl_noe_concat_model = make_diff_model(file_dict,region_key,'concat',file_length,avg_and_corr,total_unit_num)
    ...
    #TODO
    print 'div nl separate multiply JKJK avg response separate'   
    div_nl_separate_multiply_aft_cue_model = make_div_nl_avg_response_separate_model(file_dict,region_key,'aft_cue',file_length,avg_and_corr,total_unit_num)
    div_nl_separate_multiply_bfr_res_model = make_div_nl_avg_response_separate_model(file_dict,region_key,'bfr_res',file_length,avg_and_corr,total_unit_num)
    div_nl_separate_multiply_aft_res_model = make_div_nl_avg_response_separate_model(file_dict,region_key,'aft_res',file_length,avg_and_corr,total_unit_num)
    div_nl_separate_multiply_res_win_model = make_div_nl_avg_response_separate_model(file_dict,region_key,'res_win',file_length,avg_and_corr,total_unit_num)
    div_nl_separate_multiply_concat_model = make_div_nl_avg_response_separate_model(file_dict,region_key,'concat',file_length,avg_and_corr,total_unit_num)
    ...
    for win_key,win_val_x in div_nl_separate_multiply_model_return.iteritems():
        ...
        comp_r2_sig_max_min[:,1][np.isnan(comp_r2_sig_max_min[:,1])] = 0

        comp_r2_adj_all_max_min[:,0][np.isnan(comp_r2_adj_all_max_min[:,0])] = 1
        comp_r2_adj_all_max_min[:,1][np.isnan(comp_r2_adj_all_max_min[:,1])] = 0
        comp_r2_adj_sig_max_min[:,0][np.isnan(comp_r2_adj_sig_max_min[:,0])] = 1
        comp_r2_adj_sig_max_min[:,1][np.isnan(comp_r2_adj_sig_max_min[:,1])] = 0

        comp_r2_all_max_min[:,0][np.isinf(comp_r2_all_max_min[:,0])] = 1
        comp_r2_all_max_min[:,1][np.isinf(comp_r2_all_max_min[:,1])] = 0
        comp_r2_sig_max_min[:,0][np.isinf(comp_r2_sig_max_min[:,0])] = 1
        comp_r2_sig_max_min[:,1][np.isinf(comp_r2_sig_max_min[:,1])] = 0

        comp_r2_adj_all_max_min[:,0][np.isinf(comp_r2_adj_all_max_min[:,0])] = 1
        comp_r2_adj_all_max_min[:,1][np.isinf(comp_r2_adj_all_max_min[:,1])] = 0
        comp_r2_adj_sig_max_min[:,0][np.isinf(comp_r2_adj_sig_max_min[:,0])] = 1
        comp_r2_adj_sig_max_min[:,1][np.isinf(comp_r2_adj_sig_max_min[:,1])] = 0
        ...
        for x in range(np.shape(comp_r2_all)[0]):
            comp_r2_all[x] = comp_r2_all[x][~np.isnan(comp_r2_all[x])]
            comp_r2_sig[x] = comp_r2_sig[x][~np.isnan(comp_r2_sig[x])]
        ...
        try:
            axarr[0].hist(comp_r2_all[0][~is_outlier(comp_r2_all[0])],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[0].set_xlim([comp_r2_all_range[0],comp_r2_all_range[1]])
        axarr[0].set_title('r only',fontsize='small')
        try:
            axarr[1].hist(comp_r2_all[1][~is_outlier(comp_r2_all[1])],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[1].set_title('p only',fontsize='small')
        try:
            axarr[2].hist(comp_r2_all[2][~is_outlier(comp_r2_all[2])],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[2].set_title('linear',fontsize='small')
        try:
            axarr[3].hist(comp_r2_all[3][~is_outlier(comp_r2_all[3])],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[3].set_title('difference',fontsize='small')
        try:
            axarr[4].hist(comp_r2_all[4][~is_outlier(comp_r2_all[4])],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[4].set_title('div nl Y',fontsize='small')
        try:
            axarr[5].hist(comp_r2_all[5][~is_outlier(comp_r2_all[5])],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[5].set_title('separate add',fontsize='small')
        try:
            axarr[6].hist(comp_r2_all[6][~is_outlier(comp_r2_all[6])],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[6].set_title('div nl avg response separate',fontsize='small')
        for axi in axarr.reshape(-1):
        ...
        try:
            axarr[0].hist(comp_r2_sig[0],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[0].set_xlim([comp_r2_sig_range[0],comp_r2_sig_range[1]])
        axarr[0].set_title('r only',fontsize='small')
        try:
            axarr[1].hist(comp_r2_sig[1],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[1].set_title('p only',fontsize='small')
        try:
            axarr[2].hist(comp_r2_sig[2],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[2].set_title('linear',fontsize='small')
        try:
            axarr[3].hist(comp_r2_sig[3],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[3].set_title('difference',fontsize='small')
        try:
            axarr[4].hist(comp_r2_sig[4],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[4].set_title('div nl Y',fontsize='small')
        try:
            axarr[5].hist(comp_r2_sig[5],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[5].set_title('separate add',fontsize='small')
        try:
            axarr[6].hist(comp_r2_sig[6],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[6].set_title('div nl avg response separate',fontsize='small')
        for axi in axarr.reshape(-1):
        ...
        try:
            axarr[0].hist(comp_r2_adj_all[0][~is_outlier(comp_r2_adj_all[0])],color='midnightblue',lw=0)
        except:
            pass
        axarr[0].set_xlim([comp_r2_adj_all_range[0],comp_r2_adj_all_range[1]])
        axarr[0].set_title('r only',fontsize='small')
        try:
            axarr[1].hist(comp_r2_adj_all[1][~is_outlier(comp_r2_adj_all[1])],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[1].set_title('p only',fontsize='small')
        try:
            axarr[2].hist(comp_r2_adj_all[2][~is_outlier(comp_r2_adj_all[2])],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[2].set_title('linear',fontsize='small')
        try:
            axarr[3].hist(comp_r2_adj_all[3][~is_outlier(comp_r2_adj_all[3])],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[3].set_title('difference',fontsize='small')
        try:
            axarr[4].hist(comp_r2_adj_all[4][~is_outlier(comp_r2_adj_all[4])],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[4].set_title('div nl Y',fontsize='small')
        try:
            axarr[5].hist(comp_r2_adj_all[5][~is_outlier(comp_r2_adj_all[5])],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[5].set_title('separate add',fontsize='small')
        try:
            axarr[6].hist(comp_r2_adj_all[6][~is_outlier(comp_r2_adj_all[6])],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[6].set_title('div nl avg response separate',fontsize='small')
        for axi in axarr.reshape(-1):
        ...
        try:
            axarr[0].hist(comp_r2_adj_sig[0],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[0].set_xlim([comp_r2_adj_sig_range[0],comp_r2_adj_sig_range[1]])
        axarr[0].set_title('r only',fontsize='small')
        try:
            axarr[1].hist(comp_r2_adj_sig[1],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[1].set_title('p only',fontsize='small')
        try:
            axarr[2].hist(comp_r2_adj_sig[2],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[2].set_title('linear',fontsize='small')
        try:
            axarr[3].hist(comp_r2_adj_sig[3],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[3].set_title('difference',fontsize='small')
        try:
            axarr[4].hist(comp_r2_adj_sig[4],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[4].set_title('div nl Y',fontsize='small')
        try:
            axarr[5].hist(comp_r2_adj_sig[5],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[5].set_title('separate add',fontsize='small')
        try:
            axarr[6].hist(comp_r2_adj_sig[6],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[6].set_title('div nl avg response separate',fontsize='small')
        for axi in axarr.reshape(-1):
        ...
        try:
            axarr[0].hist(comp_mse_all[0],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[0].set_xlim([comp_mse_all_range[0],comp_mse_all_range[1]])
        axarr[0].set_title('r only',fontsize='small')
        try:
            axarr[1].hist(comp_mse_all[1],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[1].set_title('p only',fontsize='small')
        try:
            axarr[2].hist(comp_mse_all[2],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[2].set_title('linear',fontsize='small')
        try:
            axarr[3].hist(comp_mse_all[3],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[3].set_title('difference',fontsize='small')
        try:
            axarr[4].hist(comp_mse_all[4],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[4].set_title('div nl Y',fontsize='small')
        try:
            axarr[5].hist(comp_mse_all[5],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[5].set_title('separate add',fontsize='small')
        try:
            axarr[6].hist(comp_mse_all[6],color='midnightblue',lw=0,bins=12)
        except:
            pass
        axarr[6].set_title('div nl avg response separate',fontsize='small')
        for axi in axarr.reshape(-1):
        ...
        try:
            axarr[0].hist(comp_mse_sig[0],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[0].set_xlim([comp_mse_sig_range[0],comp_mse_sig_range[1]])
        axarr[0].set_title('r only',fontsize='small')
        try:
            axarr[1].hist(comp_mse_sig[1],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[1].set_title('p only',fontsize='small')
        try:
            axarr[2].hist(comp_mse_sig[2],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[2].set_title('linear',fontsize='small')
        try:
            axarr[3].hist(comp_mse_sig[3],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[3].set_title('difference',fontsize='small')
        try:
            axarr[4].hist(comp_mse_sig[4],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[4].set_title('div nl Y',fontsize='small')
        try:
            axarr[5].hist(comp_mse_sig[5],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[5].set_title('separate add',fontsize='small')
        try:
            axarr[6].hist(comp_mse_sig[6],color='seagreen',lw=0,bins=12)
        except:
            pass
        axarr[6].set_title('div nl avg response separate',fontsize='small')
        for axi in axarr.reshape(-1):
